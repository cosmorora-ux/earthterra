...
_OVERRIDE_PATH = os.path.join(_THIS_DIR, "formulas.json")
...
# 스탯별 최대치. 공격/방어는 3, 나머지는 5가 기본입니다.
STAT_CAPS = {
...
def clamp_stats(stats: dict):
    """스탯 값을 STAT_CAPS 범위(0 ~ 최대치) 안으로 보정합니다. 반환값: (보정된 dict, 경고 리스트)"""
    clamped = {}
    warnings = []
    for key in STAT_KEYS:
        raw = int(stats.get(key, 0))
        cap = STAT_CAPS.get(key, 5)
        fixed = max(STAT_MIN, min(cap, raw))
        if fixed != raw:
            warnings.append(f"'{key}' 값 {raw} → 최대치 {cap}(으)로 자동 조정되었습니다.")
        clamped[key] = fixed
...
def calculate_max_hp(stats: dict) -> int:
    return HP_BASE + int(stats.get("체력", 0)) * HP_PER_VIT
...
def _roll_core_dice(stats: dict, dice_count: int) -> dict:
    """정신=재굴림 기준치, 이능=다이스 최대치(면수)를 반영하는 공용 기본 다이스 굴림."""
    ability = int(stats.get("이능", 0))
    mental = int(stats.get("정신", 0))
    sides = BASE_DICE_SIDES + ability * ABILITY_BONUS
    threshold = mental * MENTAL_FLOOR_MULTIPLIER
...
def roll_attack(stats: dict, role: str = None) -> dict:
    """공격 굴림을 수행합니다. 운영자 로그에 다이스 수식을 그대로 노출하기 위해 세부 항목을 모두 담습니다."""
    core = _roll_core_dice(stats, ATTACK_DICE_COUNT)
    ...
    atk_bonus = atk_val * ATTACK_STAT_MULTIPLIER
    subtotal = core["subtotal"] + atk_bonus

    position_match = (role == ROLE_DEALER)
    if position_match:
        crit_chance = BASE_CRIT_CHANCE + luck * LUCK_CRIT_CHANCE_MULT + mental * MENTAL_CRIT_CHANCE_MULT
        is_crit = random.randint(1, 100) <= crit_chance
    else:
        crit_chance, is_crit = 0, False
    crit_mult = BASE_CRIT_DMG + agi * AGI_CRIT_DMG + ability * ABILITY_CRIT_DMG
    ...
    return {
        ...
        "stat_val": atk_val, "stat_mult": ATTACK_STAT_MULTIPLIER, "stat_bonus": atk_bonus,
...
def roll_defense(target_stats: dict, active: bool, grantor_stats: dict = None,
                  grantor_role: str = None, grantor_name: str = None) -> dict:
    """
    방어 굴림을 수행합니다.
    ...
    passive_component = target_def_val * PASSIVE_DEFENSE_STAT_MULTIPLIER
    ...
    if not active:
        return {
            ...
            "first_rolls": [], "final_rolls": [], "rerolled": [],
            "stat_val": target_def_val, "stat_mult": PASSIVE_DEFENSE_STAT_MULTIPLIER,
    ...
    core = _roll_core_dice(g_stats, DEFENSE_DICE_COUNT)
    g_def_val = int(g_stats.get("방어", 0))
    active_bonus = g_def_val * DEFENSE_STAT_MULTIPLIER
    ...
    if position_match:
        crit_chance = DEFENSE_CRIT_BASE_CHANCE + g_luck * DEFENSE_CRIT_LUCK_MULT + g_mental * DEFENSE_CRIT_MENTAL_MULT
        is_crit = random.randint(1, 100) <= crit_chance
    else:
        crit_chance, is_crit = 0, False
    crit_mult = DEFENSE_CRIT_BASE_MULT + g_agi * DEFENSE_CRIT_AGI_MULT + g_ability * DEFENSE_CRIT_ABILITY_MULT
    ...
    return {
        ...
        "mental_threshold": core["mental_threshold"],
        "first_rolls": core["first_rolls"], "final_rolls": core["final_rolls"], "rerolled": core["rerolled"],
        "stat_val": g_def_val, "stat_mult": DEFENSE_STAT_MULTIPLIER,
...
def roll_dodge(stats: dict, multiplier: float = 1.0) -> dict:
    """
    ...
    luck = int(stats.get("행운", 0))
    agi = int(stats.get("민첩", 0))
    base_chance = DODGE_BASE_CHANCE + luck * DODGE_LUCK_MULTIPLIER + agi * DODGE_AGI_MULTIPLIER
...
def calculate_final_damage(attack_total: int, defense_total: int) -> int:
    return max(MIN_DAMAGE, attack_total - defense_total)
...
def roll_heal(stats: dict, role: str = None) -> dict:
    core = _roll_core_dice(stats, HEAL_DICE_COUNT)
    base_total = round(core["subtotal"] * HEAL_OUTPUT_MULTIPLIER)
    ...
    ability = int(stats.get("이능", 0))
    if position_match:
        crit_chance = HEAL_CRIT_BASE_CHANCE + luck * HEAL_CRIT_LUCK_MULT + mental * HEAL_CRIT_MENTAL_MULT
        is_crit = random.randint(1, 100) <= crit_chance
    else:
        crit_chance, is_crit = 0, False
    crit_mult = HEAL_CRIT_BASE_MULT + agi * HEAL_CRIT_AGI_MULT + ability * HEAL_CRIT_ABILITY_MULT
